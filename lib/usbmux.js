...
var protocol = (function() {
  ...
  function pack(payload_obj) {
    ...
    var payload_buf = new Buffer(payload_plist);
 ...
 *
 * @return {EventEmitter} - the listener object
 *
 ...
 * @type {string} - UDID
 *
 * Forwards any net.Socket errors
 * @event 'error'
 * @type {Error}
 *
 * @public
...
function createListener() {
  var req = protocol.listen;
  var conn, watcher;

  var listener = new EventEmitter();
  listener.stop = listener.end = stop;

  function start() {
    conn = net.connect(address);
  ...
  /**
   * Handles messages from usbmuxd, fires onMsgComplete callback
   * every time one whole message is completed.
   * @function
   */
  var parse = protocol.makeParser(function onMsgComplete(msg) {
    ...
    if (msg.MessageType === 'Result' && msg.Number !== 0) {
      listener.emit('error', new UsbmuxdError('Listen failed', msg.Number));
      stop();
    }
    ...
    if (msg.MessageType === 'Attached') {
      ...
      listener.emit('attached', msg.Properties.SerialNumber);
    }
    ...
    if (msg.MessageType === 'Detached') {
      ...
      Object.keys(devices).forEach(function(uuid) {
        if (devices[uuid].DeviceID === msg.DeviceID) {
          delete devices[uuid];
          listener.emit('detached', uuid);
        }
      });
    }
  });

  // The usbmuxd from libimobiledevice has different behavior from apple's usbmuxd.
  // With libimobiledevice, the usbmux daemon is *only* active when a device is
  // connected. When a device is unplugged usbmuxd is stopped and the filesystem
  // entry for the unix socket (@ /var/run/usbmuxd) is left there. but you can't
  // connect to it because the process behind it isn't running. (Note: this
  // behavior is from the udev rules set @ /lib/udev/rules.d/39-usbmuxd.rules)
  //
  // Dirty hack for this problem is to set a filesystem watcher for the usbmuxd
  // pid file so we know as soon as it's active. This is mostly just a fix for
  // linux; osx & windows are likely using apple's usbmuxd.
  //
  var useWatcher = (process.platform !== 'darwin' && process.platform !== 'win32');

  if (useWatcher) {
    watcher = chokidar.watch('/var/run/usbmuxd.pid').on('add', start);
  } else {
    start();
  }
...
function connect(deviceID, devicePort) {
  return Q.Promise(function(resolve, reject) {
    ...
    var parse = protocol.makeParser(function onMsgComplete(msg) {
      ...
      // once successful, remove listener and let client manage connection
      if (msg.MessageType === 'Result' && msg.Number === 0) {
...
function Relay(devicePort, relayPort, opts) {
  if (!(this instanceof Relay)) return new Relay(devicePort, relayPort, opts);
...
Relay.prototype._startListener = function(timeout) {
  ...
  // warning timer. just alerts if no devices are connected or if the requested
  // device is not connected
  var timer = setTimeout(function() {
    // no UDID was given and no devices found yet
    if (!self._udid && !Object.keys(devices).length) {
      self._emit('warning', new Error('No devices connected'));
    }
    // UDID was given, but that device is not connected
    if (self._udid && !devices[self._udid]) {
      self._emit('warning', new Error('Requested device not connected'));
    }
  }, timeout || 1000);

  // clears the warning timer once a/the device is connected
  function readyCheck(udid) {
    if (self._udid && self._udid !== udid) return;

    self._emit('ready', udid);
    self._listener.removeListener('attached', readyCheck);
    clearTimeout(timer);
  }

  // listen as devices are connected/disconnected from usbmuxd
  this._listener = createListener()
    .on('attached', readyCheck)
    .on('attached', self._emit.bind(this, 'attached'))
    .on('detached', self._emit.bind(this, 'detached'))
    .on('error',    self._emit.bind(this, 'error'));
};
...
Relay.prototype._startServer = function() {
  ...
  this._server = net.createServer(this._handler.bind(this))
    .on('close', self._emit.bind(this, 'close'))
...
Relay.prototype._handler = function(conn) {
  ...
  var self = this;
  var udid = this._udid || Object.keys(devices)[0];
  var deviceID = devices[udid].DeviceID;
...
function getTunnel(devicePort, opts) {
  ...
  return findDevice(opts).then(function(id) {
    return connect(id, devicePort);
  });
}
